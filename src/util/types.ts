 ...
 */
export const decorateWithLookupTypes = (
    schema: Schema,
    ...
    if (isCollection(schema)) {
        const arrayItemSchema = getArrayItemsSchema(<ArraySchema>schema),
            index = _.findIndex(
                lookupTypeValues,
                (json) => json == JSON.stringify(arrayItemSchema),
            );

        let collectionSchema = <ArraySchema>schema; // eslint-disable-line prefer-const
        if (index > -1) {
            collectionSchema.items = decorateWithType(
                lookupTypeKeys[index],
                arrayItemSchema,
            );
        }
    ...
    if (schema.properties !== undefined) {
        ...
        _.map(propertyKeys, (property: string): void => {
            ...
            properties[property] = decorateWithLookupTypes(
                propertySchema,
                lookupTypes,
            );
        });
    ...
    const index: number = _.findIndex(
        lookupTypeValues,
        (json) => json == JSON.stringify(schema),
    );
    if (index > -1) {
 ...
 */
export const getTypeFromArraySchema = (
    schemaName: string,
    schema: ArraySchema,
): string => {
    ...
    switch (itemsSchema.type) {
        case "object":
            itemSchemaType = itemsSchema.__ref
                ? itemsSchema.__ref + "[]"
                : "object[]";
            break;
        default:
 ...
 */
export const isPrimitive = (schema: Schema): boolean =>
 ...
 */
export const isTypePrimitive = (string: string): boolean =>
    _.findIndex(
        [
            "string",
            "number",
...
export const getTypesFromSchema = (
    ...
    if (isCollection(schema)) {
        ...
        return [
            ...
            ...getTypesFromSchema(
                arraySchema.items.__ref ?? "object",
                arraySchema.items,
            ), // in this case we don't need the originating shema because we've already added it
        ];
    }
    ...
    if (schema.properties !== undefined) {
        ...
        _.map(propertyKeys, (property) => {
            ...
            if (containsChildSchemas(propertySchema)) {
                Array.prototype.push.apply(
                    definitions,
                    getTypesFromSchema(property, propertySchema),
                );
            }
        ...
        return [
            {
                name: schema.__ref ? schema.__ref : schemaName,
                required,
                definition,
            },
            ...definitions,
...
export const generatePropertyDefinition = (
    ...
    if (propertyDefinition.includes("[]")) {
        ...
        if (!isTypePrimitive(propertyDefinition)) {
            propertyDefinition = getPrefixedType(
                propertyDefinition,
                typePrefix,
            );
        }
        propertyDefinition = isRequired
            ? `${typePrefix}NonEmptyArray<${propertyDefinition}>`
            : `${propertyDefinition}[]`;
...
export const generateAccessMethods = (
    ...
    if (typeof type.definition !== "object" || type.definition === undefined) {
        throw (
            "Tried to generate access methods for an invalid definition " +
            JSON.stringify(type)
        );
    }
...
export const getEnumValues = (enumArray: string[] | undefined = []): string[] =>
    enumArray;
