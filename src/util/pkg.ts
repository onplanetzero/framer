...
export const isFakerInstalled = (): boolean => {
    ...
    try {
        ...
        const result: Record<string, Record<string, string>> = JSON.parse(output);
        if (
        ...
        console.warn('Faker is recommended for usage with this library, if you wish to use the mocks, install @faker-js/faker')
        return false;
    }
}
